...
void reduccion_local_worker(int headerId, int socketCliente) {
	...
	for (i = 0; i < cantTemporales; i++) {
		apareo_archivos(path_temporal_origen, arrayTemporales[i]); //aparear archivos
	}

	//ejecutar reductor guardar resultado en el temporal que me pasa master (arrayMensajes[2])
